...
# Colocated DAPO with blockwise FP8 + persistent FP8 parameters for Qwen3.5-9B-Base.
# Hardware: 1 node of 8xH100
#
# bash examples/train/algorithms/dapo/prepare_dapo_data.sh
# bash examples/train/fp8/run_fp8_hopper_blockwise_fp8param_qwen35_9b.sh
#
...
#
# fp8_param stores the Megatron primary weights in E4M3 (the distributed
# optimizer keeps FP32 masters), cutting policy parameter storage by ~39%
# on this model. Requires the Hopper blockwise recipe with FP32 block scales.
...
MEGATRON_FP8_PARAM=true
MEGATRON_FP8_PARAM_GATHER=true
...
uv run --isolated --extra megatron -m examples.train.algorithms.dapo.main_dapo \
  ...
  trainer.policy.megatron_config.fp8_param=$MEGATRON_FP8_PARAM \
  trainer.ref.megatron_config.fp8_param=$MEGATRON_FP8_PARAM \
  trainer.policy.megatron_config.ddp_config.fp8_param_gather=$MEGATRON_FP8_PARAM_GATHER \
  ...
  trainer.run_name="fp8_hopper_blockwise_fp8param_qwen35_9b" \
