...
# Colocated GRPO training+generation for Qwen3-30B-A3B on GSM8K with Megatron and PiSSA.
...
PISSA_INIT_DIR="${PISSA_INIT_DIR:-$HOME/pissa/qwen3_30b_a3b_r64}"
...
uv run --isolated --extra megatron -m skyrl.train.entrypoints.main_base \
  ...
  trainer.run_name="gsm8k_megatron_tp${MEGATRON_TP}_pp${MEGATRON_PP}_cp${MEGATRON_CP}_ep${MEGATRON_EP}_etp${MEGATRON_ETP}_${SOURCE_MODEL}_pissa_r${LORA_RANK}_a${LORA_ALPHA}" \
  trainer.resume_mode=from_path \
  trainer.resume_path="$PISSA_INIT_DIR/global_step_0" \
  trainer.ckpt_path="$HOME/ckpts/gsm8k_megatron_pissa_ckpt" \
  $@
