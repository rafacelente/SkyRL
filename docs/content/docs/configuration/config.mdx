...
megatron_config:
  ...
  lora_config:
    ...
    lora_type: "lora"
  hf_export_config:
    # Fan Megatron-to-HF safetensors export writes across ranks instead of writing the model from rank 0.
    # NOTE: for sharded multi-node exports, use a shared filesystem path.
    # Node-local paths and cloud paths can split shards across per-rank local work directories.
    # Megatron-Bridge will build the safetensors index.json based on node-local files on rank 0,
    # and this can be incorrect if not all shard files are visible.
    distributed_save: false
    # In distributed save, only ranks 0, N, 2N, ... write shards. For one writer per node,
    # set this to the number of Megatron worker GPUs per node.
...
- `megatron_config.hf_export_config.distributed_save`: Whether to parallelize Megatron-to-HuggingFace safetensors export writes across ranks. This only affects HF model exports from `hf_save_interval` or explicit `save_hf_model` calls.
- `megatron_config.hf_export_config.save_every_n_ranks`: Rank stride for distributed HF export writers. When `distributed_save=true`, ranks `0`, `N`, `2N`, ... write shards, where `N` is `save_every_n_ranks`. Set this to the number of Megatron worker GPUs per node for one writer per node. Must be at least `1`; ignored when `distributed_save=false`.

<Callout type="warn">
For sharded multi-node HF exports with `megatron_config.hf_export_config.distributed_save=true`, set `trainer.export_path` to a shared filesystem path visible to all Megatron ranks. Node-local paths and cloud paths can split shards across per-rank local work directories. Megatron-Bridge builds `model.safetensors.index.json` from the shard files visible to rank 0, so the index can be incomplete if rank 0 cannot see every shard written by the other ranks.
</Callout>
